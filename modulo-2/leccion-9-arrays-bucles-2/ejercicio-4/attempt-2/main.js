...
function makeList() {
  ...
  for (let index = 0; index < tasks.length; index++) {
    ...
    htmlCode += `<input type="checkbox" ${checked} class="checkbox" value="${index} />`;
    htmlCode += `${tasks[index].name}</li>`;
  }
  list.innerHTML = htmlCode;
}
makeList();
console.log(list);
//   /* create li element and its contents */
//   const listItem = document.createElement('li');
//   const listItemText = document.createTextNode(`${tasks[index].name}`);
//   /* add contents to each li and then each li to ul*/
